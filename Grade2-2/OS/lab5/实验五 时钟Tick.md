...
![../_images/ARMGIC.png](https://os2024lab.readthedocs.io/zh-cn/latest/_images/ARMGIC.png)

其中nIRQ是普通中断，nFIQ是快速中断。 Arm采用的中断控制器叫做GIC，即**general interrupt controller**。gic包括多个版本，如GICv1（已弃用），GICv2，GICv3，GICv4。简单起见，我们实验将选用GICv2版本。

![../_images/gicv2-logic.png](https://os2024lab.readthedocs.io/zh-cn/latest/_images/gicv2-logic.png)
...
> [!CAUTION]
>
> 需要参照 OsGicClearInt 等函数实现 OsGicEnableInt 和 OsGicDisableInt 函数。
...
#define DAIF_DBG_BIT      (1U << 3)		//用于启用或禁用调试异常		
#define DAIF_ABT_BIT      (1U << 2)		//中止异常
#define DAIF_IRQ_BIT      (1U << 1)   //控制IRQ中断
#define DAIF_FIQ_BIT      (1U << 0)   //控制FIQ中断。
//当这些位被设置为 1 时，相应的异常或中断将被屏蔽（禁用）；当这些位为 0 时，相应的异常或中断将被启用。

#define INT_MASK          (1U << 7)		//定义了中断屏蔽位，通常用于屏蔽或允许IRQ中断
...
- 将 prt_vector.S 中的 EXC_HANDLE 5 OsExcDispatch 改为 EXC_HANDLE 5 OsHwiDispatcher，表明我们将对 IRQ 类型的异常（即中断）使用 OsHwiDispatcher 处理。![截屏2024-05-16 下午7.14.56](/Users/fanglunlin/Library/Application Support/typora-user-images/截屏2024-05-16 下午7.14.56.png)
...
- src/bsp/os_attr_armv8_external.h 
...
OS_SEC_TEXT void OsTickDispatcher(void)
{
    ...
    U64 cycle = g_timerFrequency / OS_TICK_PER_SECOND;		
  //cycle 是时钟中断的周期。g_timerFrequency 是系统的时钟频率，OS_TICK_PER_SECOND 是系统每秒的tick数。这个周期用于设置系统计时器的下一次中断。
    OS_EMBED_ASM("MSR CNTP_TVAL_EL0, %0" : : "r"(cycle) : "memory", "cc"); //设置中断周期
...
OS_SEC_L0_TEXT uintptr_t PRT_HwiUnLock(void) //src/arch/cpu/armv8/common/hwi/prt_hwi.c
{
    ...
    OS_EMBED_ASM(
        "mrs %0, DAIF      \n"			//获取DAIF寄存器的当前值
        "msr DAIFClr, %1   \n"			//清除DAIF寄存器中的IRQ位（中断屏蔽位）
        : "=r"(state)
        : "i"(DAIF_IRQ_BIT)
        : "memory", "cc");

    return state & INT_MASK;		//函数返回 state 与 INT_MASK 的按位与结果，确保只返回IRQ位的状态。
}
...
OS_SEC_L4_TEXT void OsGicDisableInt(U32 intId)
{
    // Interrupt Clear-Enable Registers
  	// 计算相应寄存器的偏移量
    U32 regOffset = GICD_ICENABLERn + (intId / GICD_ICENABLER_SIZE) * sizeof(U32);
    // 计算寄存器内的位位置
    U32 bitPosition = intId % GICD_ICENABLER_SIZE;
    // 设置相应的位以禁用中断
    GIC_REG_WRITE(regOffset, BIT(bitPosition));
}
...
OS_SEC_L4_TEXT void OsGicEnableInt(U32 intId)
{
    // Interrupt Set-Enable Registers
  	// 计算相应寄存器的偏移量
    U32 regOffset = GICD_ISENABLERn + (intId / GICD_ISENABLER_SIZE) * sizeof(U32);
    // 计算寄存器内的位位置
    U32 bitPosition = intId % GICD_ISENABLER_SIZE;
    // 设置相应的位以启用中断
    GIC_REG_WRITE(regOffset, BIT(bitPosition));
}
...
实现两个函数再次构建项目并执行，输出了十次时钟中断的次数

![截屏2024-05-17 下午9.28.44](re.png)

## 心得体会

通过本次实验，我对时钟中断在操作系统中的作用和实现有了更深入的理解。时钟中断是一种由操作系统计时器周期性触发的中断，它对操作系统的多任务调度、时间管理、性能监控、节能策略以及实时系统的定时任务执行等关键功能至关重要，是确保计算机系统高效、稳定运行的基础。在实验过程中，我了解到时钟中断处理函数的实现方法，在触发时钟中断后，时钟中断异常处理函数会保存中断上下文，重新设置的中断周期。此外，通过编写和调试时钟中断的代码，我掌握了操作系统触发时钟中断的过程，增强了我对操作系统底层机制的认知和编程能力。这些经验为我今后处理更复杂的并发问题打下了坚实的基础。

## 出现的问题

主要的问题是作业中实现缺失的 OsGicEnableInt 和 OsGicDisableInt 函数，要通过观察OsGicIntClear是如何清除中断位，使用类似的方法启用和禁用相应的中断位，先找到相应的GICD\_或GICC\_寄存器并将其特定的位设置成1或0标记启用和禁用。