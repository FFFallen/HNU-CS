...
任务调度是操作系统的核心功能之一。 UniProton实现的是一个单进程支持多线程的操作系统。在UniProton中，一个任务表示一个线程。UniProton中的任务为**抢占式调度机制**，而非时间片轮转调度方式。高优先级的任务可打断低优先级任务，低优先级任务必须在高优先级任务挂起或阻塞后才能得到调度。
...
LIST_OBJECT_INIT初始化一个链表对象，使其成为自己的前驱和后继，从而创建一个空的双向链表或一个只包含单个元素的循环列表。

INIT_LIST_OBJECT初始化一个链表，用 `object` 初始化，使其成为双向循环链表中的一个节点。
...
任务相关的头文件主要包括 src/include/prt_task.h和 src/include/prt_task_external.h 两个头文件。此外还会用到 src/include/prt_module.h和 src/include/prt_errno.h 两个头文件。 prt_module.h中主要是一些模块ID的定义，而 prt_errno.h 主要是错误类型的相关定义，引入这两个头文件主要是为了保持接口与原版 UniProton 相一致。
...
其中头文件 src/include/prt_asm_cpu_external.h 包含内核相关的一些状态定义。
...
`OsMemAllocAlign`简单实现分配任务栈空间，规定大小为4KB。

它遍历一个数组 `stackMemUsed`，该数组用于跟踪已经分配的内存块。

如果找到一个未被使用的块（`stackMemUsed[i] == 0`），将其标记为已使用（`stackMemUsed[i] = 1`），并返回该块的起始地址 `&(stackMem[i][0])`。

如果所有块都已被使用，则最终返回 `NULL`，表示没有可用的内存空间。

`OsTskMemAlloc`调用`OsMemAllocAlign`函数为任务分配栈空间。

### 任务栈初始化

在理论课程中，我们知道**当发生任务切换时会首先保存前一个任务的上下文到栈里，然后从栈中恢复下一个将运行任务的上下文。**可是当任务第一次运行的时候怎么恢复上下文，之前从来没有保存过上下文？
...
void *OsTskContextInit(U32 taskID, U32 stackSize, uintptr_t *topStack, uintptr_t funcTskEntry)
{
    ...
    stack->elr = funcTskEntry;	//异常链接寄存器 elr，存储异常处理后的返回地址。
    stack->esr = 0;
    stack->far = 0;
    stack->spsr = 0x305;    		// EL1_SP1 | D | A | I | F
  //保存程序状态寄存器 spsr，这里设置为 0x305，表示进入EL1的模式，以及禁用中断（I位）和启用FIQ（F位）。
    return stack;
}
```
...
这个函数有几个有趣的地方。（1）你找不到类似 OsTskEntry(taskId); 这样的对 OsTskEntry 的函数调用。这实际上是在通过 OsTskContextInit 函数进行栈初始化时传入的，也就意味着**当任务第一次就绪运行时会进入 OsTskEntry 执行**。（2）用户指定的 taskcb->taskEntry 不一定要求是 4 参数的，可以是 0~4 参数之间任意选定。
...
检查在给定的基地址 `base` 上分配一个大小 `size`的空间时，是否会导致地址溢出，如果 `base + size` 的结果小于 `base`，则表示发生了溢出，并返回 `true`。
...
OsTaskCreateRsrcInit检查如果用户未提供堆栈空间，则通过 OsTskMemAlloc 为新建的任务分配堆栈空间；
...
OsTskCreateTcbInit用 initParam 参数等初始化任务控制块，包括栈指针、入口函数、优先级和参数等；
...
将任务栈初始化，这过程像刚发生过中断，设置任务执行所需的上下文。当任务首次创建并准备运行时，它需要一个初始的执行环境，这通常包括程序计数器（PC）、栈指针（SP）、以及其他必要的寄存器和状态信息，就好比产生中断。
...
Idle背景任务的入口函数，Idle任务顾名思义，该任务执行一个空操作的无限循环，以确保CPU在没有其他任务时保持忙碌等待状态。这种设计使得系统能够高效地管理CPU资源，并在多任务环境中保持响应性。
...
```c
任务调度是操作系统中确保合理分配处理器资源、优化系统性能、提升响应速度、满足实时性需求、保障任务公平执行以及降低能耗的重要机制，它通过智能地决定任务执行顺序和时间，从而提高系统的整体效率和用户满意度。#include "prt_typedef.h"
#include "prt_tick.h"
#include "prt_task.h"
...
以下是任务系统初始化、任务创建、任务激活、任务调度的函数调用过程：

![cddbee56508b432c64ddcfb72965db93](/Users/fanglunlin/Library/Containers/com.tencent.qq/Data/Library/Application Support/QQ/nt_qq_66ef727a86e2e9d6a5a62602f731bc18/nt_data/Pic/2024-06/Ori/cddbee56508b432c64ddcfb72965db93.jpg)

![6118f3bb6a3d23c2469d31e0ec5905ef](/Users/fanglunlin/Library/Containers/com.tencent.qq/Data/Library/Application Support/QQ/nt_qq_66ef727a86e2e9d6a5a62602f731bc18/nt_data/Pic/2024-06/Ori/6118f3bb6a3d23c2469d31e0ec5905ef.jpg)

![image-20240603下午91817928](/Users/fanglunlin/Library/Application Support/typora-user-images/image-20240603下午91817928.png)
...
> 将新建文件加入构建系统
>
> src/kernel/task 目录下加入 CMakeLists.txt，src/ 和src/kernel下的CMakeLists.txt需修改
>
> src/kernel/task/CMakeLists.txt 类似 src/bsp/CMakeLists.txt
>
> src/kernel/CMakeLists.txt 增加子目录： add_subdirectory(task)
>
> src/CMakeLists.txt 需修改增加编译目标：`$<TARGET_OBJECTS:task>`
...
> 分时调度的调度点存在于时钟Tick中断、任务结束等处。
 ...
 任务调度时机点有两个，分别时钟中断和任务退出，后者已经包含了任务调度。 发生时钟中断时调度任务有个关键点，即应在清除中断后再调度任务，否则后续时钟中断不起作用。 如图一，我在OsHwiDispatch中调用OsTskSchedule，而不是在OsTickDispatcher中。 这里实现的是分时调度中最简单的轮转调度。 有一个全局变量time_slicing，大小是时钟中断周期cycle的整数倍。 每发生一次时钟中断，在OsTickDispatcher中削减time_slicing。 如图三，调度策略实现在OsTskRR_Set中，若时间片为0，则重置时间片并切换到队列中下一任务。 另外，我删除了main中的OsActivate，因为任务调度可以由时钟中断激活。

<img src="https://qn-next.xuetangx.com/17167032643153.png" alt="17167032643153" style="zoom:43%;" />

<img src="https://qn-next.xuetangx.com/17167037122370.png" alt="17167037122370" style="zoom:43%;" />

<img src="https://qn-next.xuetangx.com/17167040182865.png" alt="17167040182865" style="zoom:43%;" />

以及一种模拟时钟中断的方法，因为只有两个任务，所以每次在每个任务的入口函数自己定义一个时间片（这里设置为5），执行完后调度一次，在调度算法中，我们可以降低该任务的优先级，这样下一次就会选择另外一个任务，看起来像是实现了轮转算法
...
void Test1TaskEntry()
{
    ...
    while (cnt > 0) {
        // PRT_TaskDelay(200);
        U64 tick=PRT_TickGetCount();
        PRT_Printf("task 1 run ...current tick:%u\n",tick);
        cnt--;
        if(cnt%5==0)
        {
            OsTskSchedule();
...
void Test2TaskEntry()
{
    ...
    while (cnt > 0) {
        // PRT_TaskDelay(100);
        U64 tick=PRT_TickGetCount();
        PRT_Printf("task 2 run ...current tick:%u\n",tick);
        cnt--;
        if(cnt%5==0)
        {
            OsTskSchedule();
...

{
    ...
    if ((g_highestTask != RUNNING_TASK) && (g_uniTaskLock == 0)) {
        ...
        if (OS_INT_INACTIVE) { // 不在中断上下文中，否则应该在中断返回时切换
            PRT_Printf("----------change task!----------the priority of the former task is %d\n",RUNNING_TASK->priority);
            OsTaskTrap();
            return;
        }
...
通过本次实验，我对任务调度在操作系统中的作用和实现有了更深入的理解。任务调度是操作系统中确保合理分配处理器资源、优化系统性能、提升响应速度、满足实时性需求、保障任务公平执行以及降低能耗的重要机制，它通过智能地决定任务执行顺序和时间，从而提高系统的整体效率和用户满意度。实验过程中，我了解到了任务创建的过程，以及任务调度的前提、和实现方法，最后还尝试实现了时间片轮转调度方法，增强了我对操作系统底层机制的认知和编程能力。这些经验为我今后处理更复杂的并发问题打下了坚实的基础。

## 出现的问题

主要问题出现在作业实现分时调度，如不了解时钟Tick中断的调度点，以及处理调度的方法，最后是在讨论区根据同学分享的思路实现的轮转调度。
