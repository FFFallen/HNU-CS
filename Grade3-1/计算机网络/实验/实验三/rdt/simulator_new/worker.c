 ...
 */
  ...
  while (true) {
	...
	if ((ww = write(mwfd, &word, TICK_SIZE)) != TICK_SIZE) print_statistics();
	if ((rr = read(mrfd, &ct, TICK_SIZE)) != TICK_SIZE) print_statistics();
 ...
 */
  ...
  if (frct >= 0) {
	...
	while(1) {
		tmp = read(prfd, inp, FRAME_SIZE);
		if(tmp <= 0) break;
		count += tmp;
		inp++;
		if (inp == &queue[MAX_QUEUE]) inp = queue;
		if (count/FRAME_SIZE >= k) break;
	}
	k = count/FRAME_SIZE;
 ...
 */
  ...
  if (debug_flags & SENDS) {
	printf("Tick %u. Proc %d sent frame to fd %d: ", tick/DELTA, id, fd);
	fr(s);
  }
}
